...
impl CommandPacket {
  ...
  fn find_crc(&mut self) -> u16{

    let mut newCrc = 0xFFFF;
    ...
    for byte in self.data.iter() {
      newCrc = (newCrc >> 8) ^ crcLookupTable[((newCrc ^ (*byte as u16)) & 0xff) as usize];
    }
    // 16 bit crc ..1's complement
    !newCrc
  }

  fn set_crc(&mut self){
    let newCrc = self.find_crc();
    // lsb first
    self.data.push((newCrc & 0x00ff) as u8);
    self.data.push((newCrc >> 8) as u8);
  }

  fn output (&mut self){
    println!("type: {:?}", self.data);
  }

  fn assemble(&mut self)  {
    let length = self.data.len();
    self.data.insert(1,length as u8 - 1);
    self.set_crc();
  }
...
fn main() {
  ...
  for arg in env::args_os().skip(1) {
    ...
    some_packet.output();
    interact(&mut port, &some_packet).unwrap();
